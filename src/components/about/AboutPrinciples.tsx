        ...
        {aboutPrinciples.map((principle, i) => (
          <AnimatedSection key={principle.title} delay={0.1 * i}>
            <StarBorder variant="accent" speed="10s" className="rounded-2xl h-full">
              <div className="flex flex-col items-center text-center gap-4 rounded-2xl bg-[var(--bg-surface)] p-6 md:p-8 h-full">
                {/* Number badge */}
                <span
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[var(--brand-accent)] text-sm font-bold text-white"
                  aria-hidden="true"
                >
                  {i + 1}
                </span>
