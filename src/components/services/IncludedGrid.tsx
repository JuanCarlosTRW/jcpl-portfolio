  ...
  return (
    <SectionWrapper id="included" variant="surface">
      <Reveal className="text-center mb-10">
        ...
        <p className="text-cg-body max-w-lg mx-auto text-[0.95rem] leading-relaxed">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 max-w-4xl mx-auto">
        {servicesIncluded.map((item, i) => (
          <Reveal key={item.title} delay={0.05 * i}>
            <div className="group flex items-start gap-4 p-5 rounded-xl border border-[rgba(255,255,255,0.06)] bg-cg-section-b transition-all duration-300 hover:border-[rgba(37,99,235,0.2)] hover:bg-[#0D1A2D] h-full">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[rgba(37,99,235,0.08)] border border-[rgba(37,99,235,0.12)] text-cg-accent shrink-0 transition-colors duration-300 group-hover:bg-[rgba(37,99,235,0.12)]">
                {icons[i]}
              </div>
              <div>
                <p className="text-sm font-semibold text-white mb-1">{item.title}</p>
                <p className="text-xs text-cg-secondary leading-relaxed">{item.description}</p>
              </div>
            </div>
          </Reveal>
        ))}
